...
//	 broker_Name in ('5ULB', 'FDMB', 'PFLB')
//	 @Query(nativeQuery = true, value="SELECT * FROM CSTracker where CAST(created_Timestamp as date) > '2022-03-31' union  SELECT * FROM CSTracker_17Events where CAST(created_Timestamp as date) > '2022-03-31' order by created_Timestamp desc ")
	 @Query("SELECT t FROM CSTracker t where t.Updated_Timestamp >= GETDATE() - 35 and broker_Name in ('5ULB', 'FDMB') and t.reference_number in (:reference_numberVal)  order by t.Updated_Timestamp desc ")  
	 List<CSTracker> fetchCSTrackDetailsByRefNum(@Param("reference_numberVal") String[] reference_numberVal);
...
//	 broker_Name in ('5ULB', 'FDMB', 'PFLB')
	 @Query("SELECT t FROM CSTracker t where t.Updated_Timestamp >= GETDATE() - 35 and t.systemStatus = :statusVal and t.reference_number in (:reference_numberVal) and broker_Name in ('5ULB', 'FDMB') order by t.Updated_Timestamp desc ")
	 List<CSTracker> fetchCSTrackDetailsByStatusRefNum(@Param("statusVal") String statusVal, @Param("reference_numberVal") String[] reference_numberVal );
